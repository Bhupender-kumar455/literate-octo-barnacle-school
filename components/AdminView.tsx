...
const AdminView: React.FC<{ currentView: string; user: User }> = ({ currentView, user }) => {
    ...
    const fetchTeachers = async () => {
        try {
            const res = await fetch(`${API_BASE_URL}/api/admin/teachers?school_id=${user.school_id}`, {
                method: 'GET',
                headers: {
    ...
    const handleSaveTeacherChanges = () => {
        if (selectedTeacher) {
            // In a real app, we would make an API call here.
            // We update the selectedTeacher state to reflect changes immediately in the View mode.
            setSelectedTeacher({
                ...selectedTeacher,
                name: editForm.name,
                    ...
                    {teachers.map(teacher => (
                        <Card key={teacher.id} className="group">
                            <div className="flex items-start justify-between mb-4">
                                <img src={teacher.logo} alt={teacher.name} className="w-16 h-16 rounded-full object-cover border-2 border-white dark:border-slate-700 shadow-sm" />
                                {(() => {
                                    const status = teacher.is_active;
                                    let text = '';
                                    let variant = '';
                                    if (status === 1) { text = 'Active'; variant = 'success'; }
                                    else if (status === 2) { text = 'Inactive'; variant = 'danger'; }
                                    else if (status === 3) { text = 'Suspended'; variant = 'warning'; }
                                    else { text = 'Unknown'; variant = 'secondary'; }
                                    return <Badge variant={variant}>{text}</Badge>;
                                })()}
                            </div>
                            <h3 className="text-lg font-bold text-slate-900 dark:text-white">{teacher.name}</h3>
                            <p className="text-sm text-slate-500 mb-4">{teacher.department[0]}</p>
                                ...
                                <Button
                                    ...
                                    onClick={() => {
                                        ...
                                        setEditForm({
                                            ...
                                            email: teacher.email,
                                            phone: teacher.phone || '',
                                            subject: 'Mathematics, Advanced Potions', // Mock default data
                                            classes: '10-A, 11-B', // Mock default data
                                            joinDate: '2015-08-15' // Mock default data
                                        });
                                    }}
                                >
                                    View Profile
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-slate-200 dark:border-slate-800 relative">
                            ...
                            <div className="flex flex-col items-center mb-6">
                                <img
                                    ...
                                    className="w-24 h-24 rounded-full object-cover border-4 border-white dark:border-slate-800 shadow-lg mb-4"
                                />
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                                        <p className="text-xs text-slate-500 uppercase font-semibold mb-1">Classes Assigned</p>
                                        {isEditingTeacher ? (
                                            <Input
                                            ...
                                            <p className="text-sm font-medium">{editForm.classes}</p>
                                        )}
                                    </div>
                                    <div className="p-3 rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800">
                            ...
                            <div className="flex gap-3 mt-8">
                                {isEditingTeacher ? (
                                    <>
                                        <Button variant="outline" className="flex-1" onClick={() => setIsEditingTeacher(false)}>Cancel</Button>
                                        <Button className="flex-1" icon={Save} onClick={handleSaveTeacherChanges}>Save Changes</Button>
                                    </>
                                ) : (
                                    <>
                                        <Button variant="outline" className="flex-1" onClick={() => setSelectedTeacher(null)}>Close</Button>
                ...
                {showCreateClassModal && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg p-6 shadow-2xl border border-slate-200 dark:border-slate-800">
                            ...
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <Input label="Grade Level" placeholder="e.g. 10" />
                                    <Input label="Section" placeholder="e.g. A" />
                                </div>
                                <Input label="Academic Year" placeholder="e.g. 2025-2026" />
                                ...
                                <div>
                                    ...
                                    <Input placeholder="e.g. Room 304" />
                                </div>
                            </div>
                            <div className="flex justify-end gap-3 mt-8">
